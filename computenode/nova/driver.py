...
class LibvirtDriver(driver.ComputeDriver):
	...
	#Added By veera
	libvirt_utils.chown(basepath(), 'root')
        utils.execute('chmod', '0777', basepath())
	...
	#Added by veera
        ...
        controller = config.LibvirtConfigGuestSerialController()
        guest.add_device(controller)
        
        ##For channel   
        channel = config.LibvirtConfigGuestChannel()
        channel.type = "unix"
        channel.source_mode = "bind"
        channel.source_path = os.path.join(FLAGS.instances_path,
                                              instance['name'],
                                              "port")
