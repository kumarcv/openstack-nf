...
LOG = logging.getLogger('quantum.extensions')
        ...
        for f in os.listdir(path):
            try:
                ...
                if file_ext.lower() == '.py' and not mod_name.startswith('_'):
                    ...
                    LOG.debug(_('ext_name = "%(ext_name)s" class name "%(new_ext_class)" in file %(file)'),{'ext_name': ext_name,'file': ext_path,'new_ext_class':new_ext_class})
...
class PluginAwareExtensionManager(ExtensionManager):
    ...
    def _plugins_support(self, extension):
        ...
        # (Trinath) added support from Grizzly to support multiple plugins
    ...
    def _plugins_implement_interface(self, extension):
        if(not hasattr(extension, "get_plugin_interface") or
           extension.get_plugin_interface() is None):
            return True
        # (Trinath) Added handling multiple plugins view.
        for plugin in self.plugins.values():
